...
export const classSchema = z.object({
  ...
  gradeLevel: z.coerce.number().int().min(1, { message: "Grade is required!" }),
...
const subjectIdsSchema = z.preprocess(
  (value) => {
    if (!value) {
      return [];
    }
...
export const teacherSchema = z
  .object({
    ...
    subjects: subjectIdsSchema,
  })
  .superRefine(requirePasswordOnCreate);
...
export const resultSchema = z.object({
  ...
  assessmentId: z.string().min(1, { message: "Assessment is required!" }),
});
